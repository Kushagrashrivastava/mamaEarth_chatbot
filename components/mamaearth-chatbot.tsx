...
const GREETING = "Hi, I'm Mama! I can help with skincare, haircare, or baby care — what's on your mind today?"
...
// (run that in the browser console to see what's installed, e.g. "Google UK English Female").
// Leave empty to fall back to the automatic female-voice guess below.
...
export function MamaearthChatbot() {
  ...
  function speakWithBrowserVoice(text: string, onDone?: () => void) {
    ...
    const preferred = PREFERRED_VOICE_NAME
  ...
  function startListening() {
    ...
    recognition.onend = () => {
      ...
      setInterimTranscript('')
      recognitionRef.current = null
      if (!gotResult && shouldListenRef.current && !mutedRef.current) {
      ...
      {open && <section className="flex h-[min(650px,calc(100vh-116px))] w-[min(390px,calc(100vw-32px))] flex-col overflow-hidden rounded-[30px] border border-[#dfead2] bg-[#fffdf8] shadow-[0_24px_80px_rgba(44,76,36,0.22)]" aria-label="Mamaearth voice support">
        ...
        {!calling && (
          <div className="relative flex min-h-0 flex-1 flex-col items-center justify-between overflow-hidden bg-gradient-to-b from-[#f1f8e9] via-[#fffdf8] to-[#f7f1df] px-6 py-7">
            ...
            <div className="relative text-center"><div className="mb-2 inline-flex items-center gap-1.5 rounded-full bg-white/80 px-3 py-1.5 text-[11px] font-semibold uppercase tracking-[0.16em] text-[#64815a]"><span className="size-1.5 rounded-full bg-[#c59b53]" />Ready when you are</div><h2 className="font-serif text-2xl text-[#29422c]">Talk to Mama</h2><p className="mt-1 text-sm text-[#6e8067]">A calm, caring conversation about your routine</p></div>
            <div className="relative size-52 rounded-full bg-[#e8f3d9] shadow-[0_16px_38px_rgba(67,108,51,0.12)] transition-all duration-700"><MascotScene /><div className="absolute bottom-2 left-1/2 flex -translate-x-1/2 items-end gap-1 rounded-full bg-white/90 px-3 py-2 shadow-sm" aria-label="Mama is ready">{[14, 24, 34, 20, 29, 17, 26].map((height, index) => <span key={index} className="w-1 rounded-full bg-[#6b9b58]" style={{ height }} />)}</div></div>
            <div className="relative w-full rounded-2xl border border-[#e4eddc] bg-white/80 p-4 text-center backdrop-blur-sm"><div className="mx-auto mb-2 flex size-9 items-center justify-center rounded-full bg-[#eef6e8] text-[#6b9b58]"><Volume2 size={17} /></div><p className="text-sm font-medium text-[#3c513d]">Get personal guidance for skincare, haircare, and more.</p><p className="mt-1 text-xs text-[#879782]">{speechSupported ? 'Your conversation stays gentle and simple' : 'Voice input isn’t supported here — you can still type once the call starts'}</p></div>
          </div>
        )}
        ...
        {calling && (
          <div className="relative flex min-h-0 flex-1 flex-col bg-gradient-to-b from-[#f1f8e9] via-[#fffdf8] to-[#f7f1df] px-5 pt-5">
            ...
            <div ref={transcriptRef} className="min-h-0 flex-1 space-y-2.5 overflow-y-auto rounded-2xl border border-[#e4eddc] bg-white/70 p-3 backdrop-blur-sm">
              ...
              {pending && (
                <div className="flex w-fit items-center gap-1.5 rounded-2xl bg-white px-3.5 py-2.5 shadow-sm" aria-label="Mama is thinking">
